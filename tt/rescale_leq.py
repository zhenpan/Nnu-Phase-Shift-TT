...
def f(x, nnu, nnu_fid, leq_fid):
	...
	A  = (R-Rf)/(R3-R1)
	trs = 4.2*leq_fid
	return A*(8.22 + 8.7e-4*x+(6*x/trs-8.22)/(math.exp(0.01*(x-trs))+1))

def g(x, nnu, nnu_fid, leq, leq_fid):
	R  = 0.23*nnu/(1.+0.23*nnu)
	Rf = 0.23*nnu_fid/(1.+0.23*nnu_fid)
        R1 = 0.23/1.23
	R3 = 0.23*3/(1+0.23*3)
...
ell1 = [l + f(l, nnu1, nnu_fid, leq_fid) for l in ell]
ell2 = [l + f(l, nnu2, nnu_fid, leq_fid) for l in ell]
ell3 = [l + f(l, nnu3, nnu_fid, leq_fid) for l in ell]
...
tt1  = [tt1[i]*g(ell[i], nnu1, nnu_fid, leq1, leq_fid) for i in range(len(ell))]
tt2  = [tt2[i]*g(ell[i], nnu2, nnu_fid, leq2, leq_fid) for i in range(len(ell))]
tt3  = [tt3[i]*g(ell[i], nnu3, nnu_fid, leq2, leq_fid) for i in range(len(ell))]

tt1_m  = [tt1_m[i]*g(ell1_m[i], nnu1, nnu_fid, leq1, leq_fid) for i in range(len(ell3_m))]
tt2_m  = [tt2_m[i]*g(ell2_m[i], nnu2, nnu_fid, leq2, leq_fid) for i in range(len(ell3_m))]
tt3_m  = [tt3_m[i]*g(ell3_m[i], nnu3, nnu_fid, leq3, leq_fid) for i in range(len(ell3_m))]
